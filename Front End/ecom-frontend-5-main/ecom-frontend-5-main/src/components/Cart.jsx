...
const Cart = () => {
        ...
        const cartItemsWithImages = await Promise.all(
          updatedCartItems.map(async (item) => {
            try {
              ...
              const imageUrl = URL.createObjectURL(response.data);
              return { ...item, imageUrl, imageBlob: response.data }; // Store imageBlob for checkout
            } catch (error) {
              console.error("Error fetching image:", error);
              return { ...item, imageUrl: "placeholder-image-url" };
            }
  ...
  const handleCheckout = async () => {
    try {
      for (const item of cartItems) {
        const { imageUrl, imageBlob, imageName, imageData, imageType, quantity, ...rest } = item; // Use imageBlob here
  
        const updatedStockQuantity = item.stockQuantity - item.quantity;
        const updatedProductData = { ...rest, stockQuantity: updatedStockQuantity };
  
        const cartProduct = new FormData();
        cartProduct.append("imageFile", imageBlob); // Attach the correct image for each product
        cartProduct.append(
          "product",
          new Blob([JSON.stringify(updatedProductData)], { type: "application/json" })
        );
